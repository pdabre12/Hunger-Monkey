...
const UpdateProfile = () => {
  ...
  axios.patch(process.env.REACT_APP_BACKEND+`profile/${localStorage.getItem("token")}`,{
     ...data
  })
  .then(res=>{
  ...
  .catch(err=>{
     ...
     seterrors(err)
        ...
        useEffect(()=>{
         ...
         axios.get(process.env.REACT_APP_BACKEND+`profile/${localStorage.getItem('token')}`).then(response=>{
