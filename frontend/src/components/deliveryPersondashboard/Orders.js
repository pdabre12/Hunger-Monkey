...
import RestoCard from './OrderCard'
...
const Orders = (props) => {
    let [originalOrders,setOriginalOrders]=useState([])
    let [ordersReceived,setOrdersReceived]=useState([])
...

{
    ...
    var headers = new Headers(); 
        ...
        // Assume the original state is an array of objects
...
let details_received= ordersReceived.map((order,index) => {
    ...
    resteraunt_name ={order.restaurant_email}
    delivery_address={order.delivery_address}
    status={order.status}
    ordered_time = {order.placed_order_time}
   ...
   <h2 className="display-4 text-center mt-5 mb-3">Pending Orders</h2>
