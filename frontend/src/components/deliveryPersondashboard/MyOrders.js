...
console.log("here is the update-------->",updated)
  function getOrdersByType(ordertype){
    let filtered_orders = []
    if (ordertype==='new'){
     filtered_orders = final_orders.filter(order=>order.status=="Pickup Ready")
     console.log('x',filtered_orders)
     setorders(filtered_orders)
    }
    else if(ordertype==='ongoing'){
    filtered_orders = final_orders.filter(order=>order.status==='Out For Delivery')
     console.log(filtered_orders)
     setorders(filtered_orders)
    }
    else if(ordertype==='past') {
     filtered_orders = final_orders.filter(order=>order.status==='Cancelled' || order.status==='Completed' ||order.status ==='Refunded')
     console.log(filtered_orders)
     setorders(filtered_orders)
    }
    else{
